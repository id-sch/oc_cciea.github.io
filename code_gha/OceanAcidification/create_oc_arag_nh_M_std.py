...
for i in range(0, num_sttn):
    ...
    # --resample by taking 'ME' means, i.e. monthly means
...
# file name and save the CSV
fn_out = '{}_M.csv'.format(file_pre)

metric_lbl = y_lbl

clmns_iea = ['year', 'month', 'time', 'data', 'error', 'SElo', 'SEup',
             'metric', 'timeseries', 'lat', 'lon', 'depth', 'order']

fn_out_csv = fun_pd_df2csvR_time(
    clmns_iea, df_list, lat_list, lon_list, depth_list, metric_lbl, ts_lbl,
