...
# clim time period
date_clim = [np.datetime64('1980-01-01'), np.datetime64('2011-01-01')]
...
clim_bgn = dates_wnt[0].astype('object').year
clim_end = dates_wnt[-1].astype('object').year
...
for i in range(num_lat_rgn):
    ...
    # only calculate the clim if file doesn't exist
    if os.path.isfile(fn_out):
        print("Thresholds for {} - {} N, {} - {} have already been calculated".format(lat_rgn1[0], lat_rgn1[1], clim_bgn, clim_end))
    else:
        print("Calculating Thresholds for {} - {} N, {} - {}".format(lat_rgn1[0], lat_rgn1[1], clim_bgn, clim_end))
        #
        ...
        for iii in range(num_ssn):
            ...
            thrsh_round = np.round(2*da2_mn_ssn.data[in_ssn])/2
            threshold1_vec[iii] = thrsh_round[0]
