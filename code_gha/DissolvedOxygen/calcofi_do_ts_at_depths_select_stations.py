...
dpth_wnt = np.array([50, 100, 150, 200])
...
# stations
df_sttn = pd.read_csv(fn_sttn)
num_sttn_all = len(df_sttn)
sttn_all = []
for i in range(num_sttn_all):
    line1 = df_sttn[var_fn_sttn[0]].values[i]*10
    sttn1 = df_sttn[var_fn_sttn[1]].values[i]*10
    sttn_all.append('{:3d}_{:3d}'.format(
...
for i in range(num_sttn):
    in_sttn_wnt = np.where(np.array(sttn_all) == sttn_wnt[i])[0]
    # bottom_depth at station
    bottom_depth_station = bottom_depth[in_sttn_wnt]
